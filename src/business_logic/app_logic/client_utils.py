...
def create_new_client(db, name, company_name, product_of_interest, email=None, tg_id=None):
    ...
    if name is None or company_name is None or product_of_interest is None:
        raise Exception("To add a client we need name, company name and the products of interest")
    ...
    if email:
        ...
        if collection.count_documents({"email": email}) > 0:
            raise Exception("Attempting to add a client with email we have in the DB. Can't add.")
    ...
    if not email and not tg_id:
        raise Exception("For new client we need email or tg id set to be able to communicate")
...
def mark_client_got_timeout(db, client, deadline):
    ...
    db[CLIENTS_COLLECTION_NAME].update_one({"_id": client["_id"]},
                                           {"$set": {"status": CLIENT_STATUS_TIMEOUT,
                                                     "deadline": deadline,
...
def mark_client_do_not_disturb(db, client):
    # add additional logic here
    db[CLIENTS_COLLECTION_NAME].update_one({"_id": client["_id"]},
                                           {"$set": {"status": CLIENT_STATUS_DONT_DISTURB,
                                                     "updated": datetime.now(tz=timezone.utc)}})
